...
gen_error_t gen_arguments_parse(const char* const restrict* const restrict arguments, const size_t* const restrict argument_lengths, const size_t argument_count, const char* const restrict short_arguments, const size_t short_argument_count, const char* const restrict* const restrict long_arguments, const size_t* const restrict long_argument_lengths, const size_t long_argument_count, gen_arguments_parsed_t* const restrict out_parsed) {
	GEN_TOOLING_AUTO gen_error_t error = gen_tooling_push("gen_arguments_parse", (void*) gen_arguments_parse, GEN_FILENAME);
	if(error.type) return error;

	if(!arguments) return gen_error_attach_backtrace(GEN_INVALID_PARAMETER, GEN_LINENO, "`arguments` was NULL");
	if(!argument_lengths) return gen_error_attach_backtrace(GEN_INVALID_PARAMETER, GEN_LINENO, "`argument_lengths` was NULL");
	if(short_argument_count && !short_arguments) return gen_error_attach_backtrace(GEN_INVALID_PARAMETER, GEN_LINENO, "`short_argument_count` was not 0 but `short_arguments` was NULL");
	if(long_argument_count && !long_arguments) return gen_error_attach_backtrace(GEN_INVALID_PARAMETER, GEN_LINENO, "`long_argument_count` was not 0 but `long_arguments` was NULL");
	if(long_argument_count && !long_argument_lengths) return gen_error_attach_backtrace(GEN_INVALID_PARAMETER, GEN_LINENO, "``long_argument_count` was not 0 but long_argument_lengths` was NULL");
	if(!out_parsed) return gen_error_attach_backtrace(GEN_INVALID_PARAMETER, GEN_LINENO, "`out_parsed` was NULL");
	...
	for(size_t i = 0; i < argument_count; ++i) {
		...
		if(argument_lengths[i] >= 3 && arguments[i][0] == '-' && arguments[i][1] == '-') {
			...
			return gen_error_attach_backtrace_formatted(GEN_NO_SUCH_OBJECT, GEN_LINENO, "Invalid argument `%sz`", arguments[i], argument_lengths[i]);
		}
...
gen_error_t gen_arguments_free_parsed(gen_arguments_parsed_t* const restrict parsed) {
	GEN_TOOLING_AUTO gen_error_t error = gen_tooling_push("gen_arguments_free_parsed", (void*) gen_arguments_free_parsed, GEN_FILENAME);
	if(error.type) return error;

	if(!parsed) return gen_error_attach_backtrace(GEN_INVALID_PARAMETER, GEN_LINENO, "`parsed` was `NULL`");
