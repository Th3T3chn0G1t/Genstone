...
gen_error_t gen_parse_args(const int argc, const char* const restrict* restrict const argv, const gen_arg_handler_t handler, const size_t n_short_args, const char* restrict short_args, const size_t n_long_args, const char* const restrict* const restrict long_args, void* const restrict passthrough) {
	GEN_TOOLING_AUTO gen_error_t error = gen_tooling_push("gen_parse_args", (void*) gen_parse_args, GEN_FILENAME);
	if(error.type) return error;

	GEN_NULL_CHECK(argv);
	if(argc < 0) gen_error_attach_backtrace(GEN_INVALID_PARAMETER, "`argc` was invalid (`argc` < 0)");
	GEN_NULL_CHECK(handler);
	if(n_short_args && !short_args) gen_error_attach_backtrace(GEN_INVALID_PARAMETER, "`short_args` was NULL but `n_short_args` > 0");
	if(n_long_args && !long_args) gen_error_attach_backtrace(GEN_INVALID_PARAMETER, "`long_args` was NULL but `n_long_args` > 0");
	...
	GEN_FOREACH_PTR(i, arg, argc - 1, argv + 1) {
		if((*arg)[0] == '-' && (*arg)[1] == '-' && n_long_args) {
			GEN_FOREACH_PTR(j, long_arg, n_long_args, long_args) {
				...
				gen_error_attach_backtrace_IF(error, "`gen_string_compare` failed");
				if(equal) {
					const char* param = NULL;
					error = gen_string_character_first(*arg, GEN_STRING_NO_BOUND, '=', GEN_STRING_NO_BOUND, &param);
					gen_error_attach_backtrace_IF(error, "`gen_string_character_first` failed");
					error = handler(GEN_ARG_LONG, j, param ? param + 1 : NULL, passthrough);
					gen_error_attach_backtrace_IF(error, "Call to arg handler failed");
				}
			}
		}
