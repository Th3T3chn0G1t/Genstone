...
int cnd_timedwait(cnd_t* cond, mtx_t* mtx, const struct timespec* abs_time) {
	int rt;
	GEN_REQUIRE_NON_NULL(mtx);
	GEN_REQUIRE_NON_NULL(cond);
	GEN_REQUIRE_NON_NULL(abs_time);
...
int thrd_create(thrd_t* thr, thrd_start_t func, void* arg) {
	...
	GEN_REQUIRE_NON_NULL(thr);
	gen_error_t error = gzalloc((void**) &pack, sizeof(struct impl_thrd_param), 1);
	GEN_REQUIRE_NO_ERROR(error);
...
void thrd_sleep(const struct timespec* time_point, struct timespec* remaining) {
	GEN_REQUIRE_NON_NULL(time_point);
