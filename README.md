...
|`GEN_INTERNAL_MICROSOFT_MAX_STRERROR`|Any valid array size|100|It was warranted. Windows CRT asks for `strerror_s` to be used in place of `strerror` but doesn't provide `strerrorlen_s`. We use this to allocate a fixed size buffer to store `strerror_s` values
